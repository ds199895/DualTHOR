...
{
    [SerializeField]
    protected List<GameObject> CurrentlyContains = new(); //当前容器内包含的对象列表。

    [SerializeField]
    public GameObject myParent; //容器对象的父对象引用。
    ...
    void OnEnable()
    {
        if (myParent == null)
        {
            ...
            Debug.Log(gameObject.transform.parent.GetComponent<SimObjPhysics>());
            if (gameObject.GetComponentInParent<SimObjPhysics>().transform.gameObject)
            {
                myParent = gameObject.GetComponentInParent<SimObjPhysics>().transform.gameObject;
            }
